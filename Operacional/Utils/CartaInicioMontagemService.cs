...
internal static class CartaInicioMontagemService
{
    internal const string Consulta = @"
        SELECT *
    ...
    public static async Task<Resultado?> GerarAsync(DateTime inicio, DateTime fim, CancellationToken cancellationToken)
    {
        if (inicio.Date > fim.Date || fim.Date == DateTime.MaxValue.Date)
            throw new ArgumentException("Periodo invalido.");
        var modelo = SistemaPathResolver.GetModeloPath("MODELO_CARTA_INICIO_MONTAGEM.docx");
        using var dados = new DataTable("qrybasemalamontagem");
        ...
        if (dados.Rows.Count == 0) return null;
        cancellationToken.ThrowIfCancellationRequested();

        var pasta = SistemaPathResolver.GetImpressosPath($"CartaInicioMontagem_{DateTime.Now:yyyyMMdd_HHmmss}_{Guid.NewGuid():N}");
        ...
        {
            // Only remove files owned by this unique, failed generation.
