...
public partial class CartaInicioMontagem : UserControl
{
    private CancellationTokenSource? geracao;
    private string? pastaGerada;

    public CartaInicioMontagem() => InitializeComponent();
    ...
    private async void OnGerarClick(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show("Informe a data inicial e a data final.", "Inicio Montagem", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        if (inicio.Date > fim.Date || fim.Date == DateTime.MaxValue.Date)
        {
            MessageBox.Show("Informe um periodo valido, com a data final igual ou posterior a inicial.", "Inicio Montagem",
        ...
        try
        {
            var resultado = await CartaInicioMontagemService.GerarAsync(inicio, fim, cancelamento.Token);
            if (resultado == null)
            {
        ...
        catch (Exception ex)
        {
            ...
            ErrorDialog.Show(ex, "Inicio Montagem");
        }
        finally
        {
            geracao = null;
    ...
    private void OnAbrirPastaClick(object sender, RoutedEventArgs e)
    {
        ...
        catch (Exception ex) { ErrorDialog.Show(ex, "Inicio Montagem"); }
    }
}
